...
export class LossProvisionComponent implements OnInit {
  ...
  async handleData(queryResult: ApolloQueryResult<QueryResult>) {
    if (!queryResult.loading) {
      ...
      for (let i in this.data) {
        if (this.data[i].label && this.data[i].label != "0x71482F8cD0e956051208603709639FA28cBc1F33") { // if it's not the weird chartjs thing
        ...
        } else if (this.data[i].label == "0x71482F8cD0e956051208603709639FA28cBc1F33") {
          // crvHUSD
          for (let j in this.timestamps) {
